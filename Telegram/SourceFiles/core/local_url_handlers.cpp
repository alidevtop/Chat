...
QString TryConvertUrlToLocal(QString url) {
	...
	auto subdomainMatch = regex_match(u"^(https?://)?([a-zA-Z0-9\\_]+)\\.teamgram\\.me(/\\d+)?/?(\\?.+)?"_q, url, matchOptions);
	if (subdomainMatch) {
	...
	auto telegramMeMatch = regex_match(u"^(https?://)?(www\\.)?(teamgram\\.(me)|teamgram\\.me)/(.+)$"_q, url, matchOptions);
	if (telegramMeMatch) {
		...
		if (const auto phoneMatch = regex_match(u"^\\+([0-9]+)(\\?|$)"_q, query, matchOptions)) {
			...
			return u"tg2://socks?"_q + socksMatch->captured(1);
